...
/** Same SSE shape as chat — `/api/plan` uses the planning system prompt server-side. */
export type PlanStreamPiece =
  | { type: "delta"; delta: string }
  | { type: "usage"; usage: ChatUsagePayload; costUsd: number | null; costInr: number | null };

export async function* streamBackendPlan(
  messages: ChatMessage[],
  conversationId?: string | null,
): AsyncGenerator<PlanStreamPiece, void, unknown> {
  const url = chatEndpoint("/api/plan");
  if (import.meta.env.DEV) {
    console.info("[plan] POST", url);
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(detail.slice(0, 400) || `Plan request failed (${res.status})`);
  }
